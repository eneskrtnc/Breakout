...
namespace SpaceTrader.Editor.Addressables
{
    public static class AddressablesFoundation
    {
        ...
        public static void RebuildFromConventions()
        {
            ...
            if (s == null)
            {
                Debug.LogError("No Addressables settings.");
                return;
            }
            ...
            var rules = new List<Rule>
            {
                // UI
                new Rule(
                    new[] { "Assets/Art/ui/icons", "Assets/Art/UI/Icons" },
                    "t:Sprite",
                    "ui/icons",
                    "UI",
                    "ui"
                ...
                // Ships
                new Rule(
                    new[] { "Assets/Art/ships/icons", "Assets/Art/Ships/Icons" },
                    "t:Sprite",
                    "ships/icons",
                    "Ships",
                    "ships"
                ...
                new Rule(
                    new[] { "Assets/Prefabs/Ships", "Assets/Ships/Prefabs" },
                    "t:Prefab",
                    "ships/prefabs",
                    "Ships",
                    "ships"
                ...
                // FX
                new Rule(
                    new[] { "Assets/Art/fx/sprites", "Assets/Art/VFX" },
                    "t:Sprite",
                    "fx/sprites",
                    "FX",
                    "fx"
                ...
                // ENV
                new Rule(
                    new[] { "Assets/Art/env/tilesets", "Assets/Art/Tiles" },
                    "t:Sprite",
                    "env/tilesets",
                    "Env",
                    "env"
            ...
            foreach (var r in rules)
            {
                foreach (var root in r.Roots.Where(AssetDatabase.IsValidFolder))
                {
                    ...
                    foreach (var guid in guids)
                    {
                        ...
                        var address = string.IsNullOrEmpty(r.AddressPrefix)
                            ? id
                            : (r.AddressPrefix + "/" + id);
                        ...
                        if (entry == null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(r.Label))
                                entry.SetLabel(r.Label, true, true);
                            added++;
                        }
                        else
                        {
                            ...
                            {
                                s.CreateOrMoveEntry(guid, group, false, false);
                                moved++;
                                ch = true;
                            }
            ...
            Debug.Log(
                string.Format(
                    "[Addr] Rebuild From Conventions → added:{0}, moved:{1}, updated:{2}",
                    added,
                    moved,
                    touched
                )
        ...
        public static void HealthCheck()
        {
            ...
            if (string.IsNullOrEmpty(lb) || string.IsNullOrEmpty(ll))
                Debug.LogWarning(
                    "[Addr] Profile vars missing. Expected LocalBuildPath & LocalLoadPath."
                );
            ...
            foreach (var g in s.groups.Where(g => g != null))
            {
                ...
                if (schema == null)
                {
                    Debug.LogWarning("[Addr] Group without Bundled schema: " + g.Name);
                    continue;
                }
                ...
                foreach (var e in g.entries)
                {
                    if (string.IsNullOrEmpty(e.address))
                        Debug.LogWarning(
                            "[Addr] Empty address in group " + g.Name + " → guid: " + e.guid
                        );
                }
            }
        ...
        static void EnsureVar(
            AddressableAssetProfileSettings p,
            string profileId,
            string name,
            string value
        )
        {
            ...
            if (existing == null)
                ...
                p.SetValue(profileId, name, value);
        }
        ...
        static AddressableAssetGroup EnsureGroup(AddressableAssetSettings s, string name)
        {
            ...
            var schema =
                g.GetSchema<BundledAssetGroupSchema>() ?? g.AddSchema<BundledAssetGroupSchema>();
