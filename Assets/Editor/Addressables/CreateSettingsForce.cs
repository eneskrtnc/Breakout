...
namespace SpaceTrader.Editor.Addressables
{
    public static class CreateSettingsForce
    {
        ...
        public static void CreateOrRepair()
        {
            ...
            EnsureVar(ps, settings.activeProfileId, "LocalBuildPath",
                "{UnityEngine.AddressableAssets.Addressables.BuildPath}/[BuildTarget]");
            EnsureVar(ps, settings.activeProfileId, "LocalLoadPath",
        ...
        static void EnsureVar(AddressableAssetProfileSettings ps, string pid, string name, string value)
        {
            var cur = ps.GetValueByName(pid, name);
            if (cur == null) ps.CreateValue(name, value);
            else if (string.IsNullOrEmpty(cur)) ps.SetValue(pid, name, value);
        }
    }
}
#endif
